...
// AES256GCM concrete struct of Crypto PII Use Case.
type AES256GCM struct {
	secret string
	pepper string
}

// NewAES256GCM is constructor.
func NewAES256GCM(secret string, pepper string) Crypto {
	return &AES256GCM{
		secret: secret,
		pepper: pepper,
	}
...
func (a AES256GCM) Encrypt(plainText string) (cipherText []byte, err error) {
...
func (a AES256GCM) Decrypt(encryptedTextBytes []byte) (plainText []byte, err error) {
	// The key argument should be the AES key, either 16 or 32 bytes
	// to select AES-128 or AES-256.
	key := []byte(a.secret)
